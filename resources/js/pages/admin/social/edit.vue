...
const {updateSocialRoute, social} = usePage().props
...
form.name = social.name
form.link = social.link

const submit = () => {
    form.put(updateSocialRoute, {
        onError: () => {
            toaster.error("Error")
        },
        onSuccess: () => {
            ...
            <div class="space-y-4">
                <form action="">
                    <div class="mb-4 flex items-center justify-between space-y-2">
                        <h1 class="text-2xl font-bold tracking-tight">Mise a jour {{social.name}}</h1>

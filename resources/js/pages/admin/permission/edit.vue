...
const {roles, permission,updatePermissionRoute} = usePage().props
const form = useForm({
    name: "",
    roles: []
})
...
form.roles = permission.roles


const updatePermission = () => {
    form.put(updatePermissionRoute, {
        onError: () => {
            ...
            <div class="space-y-4">
                <form action="">
                    <div class="mb-4 flex items-center justify-between space-y-2">
                        <h1 class="text-2xl font-bold tracking-tight">Modification de la permission {{ permission.name }}</h1>
                        <div class="flex gap-2">
                            <Button @click.prevent="updatePermission">
                                Mettre a jour
                            </Button>
                        </div>
                    </div>

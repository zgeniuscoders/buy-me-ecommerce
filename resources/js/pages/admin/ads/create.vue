...
const {addAdsRoute, addAdsToDraftRoute} = usePage().props
...
onMounted((() => {
    adsForm.start_at = startAt.value
    adsForm.end_at = endAt.value

    console.log(startAt.value.toString())
}))

const publish = () => {
    adsForm.start_at = startAt.value.toString()
    adsForm.end_at = endAt.value.toString()
    adsForm.post(addAdsRoute, {
        onError: () => {
            toaster.error("error")
        },
        onSuccess: () => {
...
const draft = () => {
    adsForm.start_at = startAt.value.toString()
    adsForm.end_at = endAt.value.toString()
    adsForm.post(addAdsToDraftRoute, {
        onError: () => {
            toaster.error("error")
        },
        onSuccess: () => {
    ...
    <admin-layout>
        <section class="container max-w-screen-lg">
            <div class="mb-4 flex items-center justify-between space-y-2">
                <h1 class="text-2xl font-bold tracking-tight">Cree une publicite</h1>
                <div class="flex gap-2">
                    <Button>Discard</Button>
                    <Button @click="draft">Save Draft</Button>
